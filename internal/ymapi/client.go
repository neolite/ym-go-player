...
type Client struct {
	...
	http *http.Client
	// stream — клиент для загрузки потоков. У него намеренно нет
...
// назначение этого метода — отдавать именно потоковый клиент: сборка
// приложения (internal/app) забирает его здесь и передаёт в stream.NewProxy,
// через него и идёт аудио. Для запросов метаданных пакет использует
// Get/PostForm/PostJSON, которые берут свой, таймаутированный клиент
// самостоятельно.
