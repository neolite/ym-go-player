...
// Exhaust уводит позицию за конец очереди: Current() после него nil.
// Нужен пропуску недоступных треков (skipUnavailable в httpapi): когда
// хвост очереди сплошь недоступен, позиция не должна оставаться на треке,
// который играть нельзя, — иначе кадр состояния покажет его «текущим»,
// и фронтенд попытается включить его и снова словит ошибку.
func (q *Queue) Exhaust() {
	q.mu.Lock()
	defer q.mu.Unlock()
	q.index = len(q.tracks)
}
